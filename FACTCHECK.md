...
| `validate_evidence.py` rejects non-URL source_url | README implied it enforced "resolving" source; code accepted `"not a url"` | No URL format check existed | **Doc promised validation that code didn't enforce** | Fixed — added `_looks_like_url()` check + tests |
...
| "16 unit tests" | `python3 -m unittest discover -s tests` → Ran 47 tests | ✅ Updated — now 47 tests (added URL-format validation tests) |
...
### Summary
- 4 errors found and fixed (2 high impact: wrong install commands; 1 doc/code mismatch: URL validation)
